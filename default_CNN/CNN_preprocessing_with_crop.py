...
import process_visualization as pv
...
images_train, labels_train = dp.crop_distorted_inputs( data_dir=data_dir,
                                                       batch_size=batch_size )
# define the pre-scaled testing image & corresponding label
images_test, labels_test = dp.inputs( eval_data=True,
                                      data_dir=data_dir,
                                      batch_size=batch_size )
...
with tf.name_scope( '1_1st_convolution_layer' ):
    ...
    pv.variable_summaries( weight1 )
    bias1 = tf.Variable( tf.constant( 0.0, shape=[64] ), name='bais1' )
    pv.variable_summaries( bias1 )
...
with tf.name_scope( '3_2nd_convolution_layer' ):
    ...
    pv.variable_summaries( weight2 )
    bias2 = tf.Variable( tf.constant( 0.1, shape=[64] ), name='bias2' )
    pv.variable_summaries( bias2 )

    kernel2 = tf.nn.conv2d( pool1, weight2, strides=[1, 1, 1, 1],
                            padding='SAME' )
...
with tf.name_scope( '5_1st_Fully_Connected_layer' ):
    ...
    pv.variable_summaries( weight3 )
    bias3 = tf.Variable( tf.constant( 0.1, shape=[384] ), name='bais3' )
    pv.variable_summaries( bias3 )
...
with tf.name_scope( '6_2nd_Fully_Connected_layer' ):
    ...
    pv.variable_summaries( weight4 )
    bias4 = tf.Variable( tf.constant( 0.1, shape=[192] ) )
    pv.variable_summaries( bias4 )
...
with tf.name_scope( '7_3rd_Fully_Connected_layer' ):
    ...
    pv.variable_summaries( weight5 )
    bias5 = tf.Variable( tf.constant( 0.0, shape=[10] ), name='bias5' )
    pv.variable_summaries( bias5 )
...
with tf.name_scope( 'init' ):
    ...
    writer = tf.summary.FileWriter( "crop_graph/", sess.graph )
...
for step in range( max_steps ):
    ...
    if step % batch_iter == 0:
        ...
        pl.plot_each( 'training accuracy per batch iteration', 'batch iteration',
                      ...
                      'result_default_CNN/Batch iteration/training accuracy_3000 .png' )
        ...
        pl.plot_each( 'training loss per batch iteration', 'batch iteration',
                      ...
                      'result_default_CNN/Batch iteration/training loss_3000 .png' )
        ...
        pl.plot_each( 'training examples per second ', 'batch iteration',
                      ...
                      'result_default_CNN/Batch iteration/training examples_per_sec_3000 .png' )
        ...
        pl.plot_each( 'training seconds per batch ', 'batch iteration',
                      ...
                      'result_default_CNN/Batch iteration/training sec_per_batch_3000 .png' )
        print( "batch iteration based graphs finished" )

        # plot the training accuracy image of each training iteration
        pl.plot_each( 'training accuracy per training iteration', 'batch iteration',
                      'accuracy', training_counter, accuracy_train,
                      'result_default_CNN/Training iteration/training accuracy_3000 .png' )
        # plot the training accuracy image of each training iteration
        pl.plot_each( 'testing accuracy per training iteration', 'batch iteration',
                      'accuracy', training_counter, accuracy_test,
                      'result_default_CNN/Training iteration/testing accuracy_3000 .png' )
...
saver.save( sess, 'default_model/CNN_preprocessing_with_crop_3000', global_step=step + 1 )
