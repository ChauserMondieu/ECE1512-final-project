...
with tf.name_scope( '5_1st_Fully_Connected_layer' ):
    # input layer: [batch_size, 32, 32, 64] hidden layer: 1536
    # reshape each tensor to 2d
    reshape = tf.reshape( pool2, [batch_size, -1] )
    dim = reshape.get_shape()[1].value

    weight3 = variable_with_weight_loss( [dim, 1536], stddev=0.04, wl=0.004, name='weight3' )
    vi.variable_summaries( weight3 )
    bias3 = tf.Variable( tf.constant( 0.1, shape=[1536] ), name='bais3' )
...
with tf.name_scope( '6_2nd_Fully_Connected_layer' ):
    # hidden layer: 768
    weight4 = variable_with_weight_loss( [1536, 768], stddev=0.04, wl=0.004, name='weight4' )
    vi.variable_summaries( weight4 )
    bias4 = tf.Variable( tf.constant( 0.1, shape=[768] ) )
...
with tf.name_scope( '7_3rd_Fully_Connected_layer' ):
    # hidden layer: 384
    weight5 = variable_with_weight_loss( [768, 384], stddev=0.04, wl=0.004, name='weight5' )
    vi.variable_summaries( weight5 )
    bias5 = tf.Variable( tf.constant( 0.1, shape=[384] ) )
    vi.variable_summaries( bias5 )

    local5 = tf.nn.relu( tf.matmul( local4, weight5 ) + bias5, name='local5_output' )

# [8] fourth fully connected layer
with tf.name_scope( '8_4th_Fully_Connected_layer' ):
    # hidden layer: 192
    weight6 = variable_with_weight_loss( [384, 192], stddev=0.04, wl=0.004, name='weight6' )
    vi.variable_summaries( weight6 )
    bias6 = tf.Variable( tf.constant( 0.1, shape=[192] ) )
    vi.variable_summaries( bias6 )

    local6 = tf.nn.relu( tf.matmul( local5, weight6 ) + bias6, name='local6_output' )

# [9] fifth fully connected layer
with tf.name_scope( '8_5th_Fully_Connected_layer' ):
    # output layer: 10
    weight7 = variable_with_weight_loss( [192, 10], stddev=1 / 192.0, wl=0.0, name='weight7' )
    vi.variable_summaries( weight7 )
    bias7 = tf.Variable( tf.constant( 0.0, shape=[10] ), name='bias7' )
    vi.variable_summaries( bias7 )

    final_output = tf.matmul( local6, weight7 ) + bias7
